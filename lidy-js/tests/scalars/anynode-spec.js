...
describe("Lidy scalars ->", function() {
    ...
    describe("any scalar : ", function() {
        ...
        it("A map value",
            function() { expect( parse({src_data: "{ un: 1, deux: 2 }", dsl_data: "main: any"}).contents.getChild(0).value).toEqual({un: 1, deux:2})})

        it("A list value",
            function() { expect( parse({src_data: '[ un, 1, deux, null ]', dsl_data: "main: any"}).contents.getChild(0).value).toEqual([ "un", 1, "deux", null ])})

        it("A null value",
            function() { expect( parse({src_data: '~', dsl_data: "main: any"}).contents.getChild(0).value).toEqual(null)})

        it("complex type",
            function() { expect( parse({src_data: "{ un: [1, { un: '1' } ], deux: 2 }", dsl_data: "main: any"}).contents.getChild(0).value).toEqual({un: [1, { un: '1'}], deux:2})})
