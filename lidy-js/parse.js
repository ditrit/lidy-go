...
function parse_lidy(ctx, rule_name, current) {      // dsl parsing of the source code
  ...
  if (rule === undefined) { 
    ctx.errors.push(new LidyError('GRAMMAR_ERROR', (ctx.src.range) ? ctx.src.range[0] : 0, `ERROR : no rule named ${rule_name} found.`))
  } else {
...
export function parse(input) { 
  ...
  ctx = parse_lidy(ctx, input.keyword, ctx.src)       // dsl parsing of the source code

  return ctx
}

// let ctx = parse({src_data: "tagada", dsl_data: "main: string"})
// console.log(ctx)
